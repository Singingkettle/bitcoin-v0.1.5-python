"""纯 Python 实现的 RIPEMD-160 哈希算法。

很多 OpenSSL 3 的构建里 hashlib.new('ripemd160') 已经不可用了，
所以 hashes.py 在那种情况下会退回到这里的实现。

算法结构：消息按 64 字节分组，每组同时走"左、右"两条各 80 步的流水线，
最后把两条线的结果交叉加回 5 个 32 位状态字。常数表照抄自算法标准。
"""
...
def ripemd160(data: bytes) -> bytes:
    ...
    # 填充（与 MD4/MD5 相同）：先补一个 0x80，再补 0 直到长度除以 64 余 56，
    # 最后追加 8 字节小端的“原始消息比特长度”
    bitlen = len(data) * 8
    data += b"\x80"
